...
const { getContextDeforestationHistory } = require("../utils/prodes-tools");
...
setReportFormat = async (
  ...
  const resultReportData = {
    ...
    currentYear,
    currentDate: `${("0" + today.getDate()).slice(-2)}/${("0" + (today.getMonth() + 1)).slice(
      -2
    )}/${currentYear}`,
  };
...
setDeterData = async (
  ...
  if (propertyData && views.DETER && type === "deter") {
    ...
    const resultDeterAreaPastDeforestation = await sequelize.query(sqlDeterAreaPastDeforestation, {
    ...
    const sqlDeforestationAlerts = `
      ...
      FROM public.${views.DETER.children.CAR_X_DETER.tableName} AS carxdeter, public.${
      views.STATIC.children.BIOMAS.tableName
    } bio
      WHERE ${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
    ...
    propertyData["deforestationAlerts"] = await sequelize.query(sqlDeforestationAlerts, {
      type: QueryTypes.SELECT,
    });
    // -----------------------------------------------------------------------------------------------------------------
    ...
    const sqlCrossings = `
      ...
      UNION ALL
        ...
        FROM public.${views.DETER.children.CAR_DETER_X_RESERVA.tableName} WHERE ${
      views.DETER.tableOwner
    }_${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
      ...
      UNION ALL
        ...
        FROM public.${views.DETER.children.CAR_DETER_X_EXPLORA.tableName} WHERE ${
      views.DETER.tableOwner
    }_${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
      ...
      UNION ALL
        ...
        FROM public.${views.DETER.children.CAR_DETER_X_DESMATE.tableName} WHERE ${
      views.DETER.tableOwner
    }_${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
      ...
      UNION ALL
        ...
        FROM public.${views.DETER.children.CAR_DETER_X_DESEMB.tableName} WHERE ${
      views.DETER.tableOwner
    }_${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
      ...
      UNION ALL
        ...
        FROM public.${views.DETER.children.CAR_DETER_X_QUEIMA.tableName} WHERE ${
      views.DETER.tableOwner
    }_${columnCarEstadual} = '${carRegister}' ${getFilterClassSearch(
      dateSql,
      filter,
...
setProdesData = async (
  ...
  if (propertyData && views.PRODES && type === "prodes") {
    ...
    propertyData["prodesRadam"] = await sequelize.query(sqlFisionomiaPRODESSum, {
      type: QueryTypes.SELECT,
    });
    // -----------------------------------------------------------------------------------------------------------------
    ...
    const sqlDeforestationHistory = ` SELECT
            ...
            GROUP BY date
            ORDER BY date`;
    const deforestationHistory = await sequelize.query(sqlDeforestationHistory, {
      type: QueryTypes.SELECT,
    });
...
setBurnedData = async (
  ...
  if (propertyData && views.BURNED && type === "queimada") {
    ...
    const sqlHistoryFireSpot = `
        ...
        `;
    propertyData["historyFireSpot"] = await sequelize.query(sqlHistoryFireSpot, {
      type: QueryTypes.SELECT,
    });
    // -----------------------------------------------------------------------------------------------------------------
  }
...
setBurnedAreaData = async (
  ...
  if (propertyData && views.BURNED_AREA && type === "queimada") {
    ...
    const restrictUseBURNEDAREASum = await sequelize.query(sqlRestrictUseBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });
    ...
    const legalReserveBURNEDAREASum = await sequelize.query(sqlLegalReserveBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });

    const indigenousLandBURNEDAREASum = await sequelize.query(sqlIndigenousLandBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });

    const explorationBURNEDAREASum = await sequelize.query(sqlExploraBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });

    const deforestationBURNEDAREASum = await sequelize.query(sqlDesmateBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });

    const embargoedAreaBURNEDAREASum = await sequelize.query(sqlEmbargoedAreaBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });

    const landAreaBURNEDAREASum = await sequelize.query(sqlLandAreaBURNEDAREASum, {
      type: QueryTypes.SELECT,
      plain: true,
    });
...
getDeforestationHistoryAndChartNdviContext = async (docDefinitionContent, reportData) => {
  moment.locale("pt-br");
...
getContentForDeforestionAlertsContext = async (
  ...
  const paragraphIndex = docDefinitionContent.findIndex(
    (item) => item.hasOwnProperty("text") && item["text"] === "DeforestionAlertsContext"
  );
  docDefinitionContent.splice(paragraphIndex, 1, deforestationAlertsContext);
  return docDefinitionContent;
};
...
getContentConclusion = async (docDefinitionContent, conclusionText) => {
  ...
  const conclusionIdx =
    docDefinitionContent.findIndex(({ text }) => text && text.includes("CONCLUSÃO")) + 1;
...
firingChartsReport = async (reportData) => {
  reportData.chartsImages["firstFiringChart"] = {
    image: await FiringCharts.historyFireSpot(reportData.property.historyFireSpot).toDataUrl(),
    fit: [450, 450],
    alignment: "center",
  };
...
module.exports.reportFormatProdes = async (
  ...
  resultReportData.images["geoserverImage1"] = getImageObject(
    await geoserverService.getMapImage({
      bbox: `${reportData["stateBBox"]}`,
      cql_filter: `geocodigo<>'';municipio='${resultReportData.property.city.replace(
        "'",
        "''"
      )}';numero_do1='${resultReportData.property.stateRegister}'`,
      format: "image/png",
  ...
  resultReportData.images["geoserverImage2"] = getImageObject(
    await geoserverService.getMapImage({
  ...
  resultReportData.images["geoserverImage3"] = getImageObject(
    await geoserverService.getMapImage({
      bbox: `${resultReportData.property.planetBBox}`,
  ...
  resultReportData.images["geoserverLegend"] = getImageObject(
    await geoserverService.getLegendImage({
      format: "image/png",
      height: "30",
  ...
  resultReportData.images["geoserverImage4"] = getImageObject(
    await geoserverService.getMapImage({
  ...
  resultReportData.images["geoserverImage5"] = getImageObject(
    await geoserverService.getMapImage({
  ...
  resultReportData.images["geoserverImage6"] = getImageObject(
    await geoserverService.getMapImage({
      bbox: `${resultReportData.property.bbox}`,
      cql_filter: `RED_BAND>0;${carColumnSema}='${resultReportData.property.gid}';gid_car='${resultReportData.property.gid}';${carColumn}='${resultReportData.property.gid}'`,
      format: "image/png",
  ...
  resultReportData.images["geoserverImage7"] = getImageObject(
    await geoserverService.getMapImage({
      bbox: `${resultReportData.property.planetBBox}`,
      cql_filter: `RED_BAND>0;${carColumnSema}='${resultReportData.property.gid}';gid_car='${resultReportData.property.gid}';${carColumn}='${resultReportData.property.gid}'`,
      format: "image/png",
...
formatValuesProdes = (resultReportData) => {
  ...
  resultReportData.property.areaPastDeforestation = formatter.formatNumber(
    resultReportData.property.areaPastDeforestation
  );
  ...
  resultReportData.property.prodesTotalArea = formatter.formatHectare(
    resultReportData.property.prodesTotalArea
  );
  resultReportData.property.areaUsoCon = formatter.formatHectare(
    resultReportData.property.areaUsoCon
  );
  resultReportData.property.prodesArea = formatter.formatHectare(
    resultReportData.property.prodesArea
  );

  if (resultReportData.property.tableVegRadam) {
    if (resultReportData.property.tableVegRadam.pastDeforestation) {
      const listPastDeforestation =
        resultReportData.property.tableVegRadam.pastDeforestation.split("\n");
      let pastDeforestationStr = "";
      for (const data of listPastDeforestation) {
        const pastDeforestation = data.substring(0, data.indexOf(":"));
        const valuePastDeforestation = formatter.formatNumber(
          data.substring(data.indexOf(":") + 1, data.length)
        );

        pastDeforestationStr = pastDeforestationStr
          ? `${pastDeforestationStr}\n${pastDeforestation}: ${valuePastDeforestation}`
          : `${pastDeforestation}: ${valuePastDeforestation}`;
      }
...
module.exports.reportFormatDeter = async (
  ...
  if (
    ...
    for (const alert of deforestationAlerts) {
      ...
      const view =
        yearBefore < 2013 ? "LANDSAT_5_" : yearBefore < 2017 ? "LANDSAT_8_" : "SENTINEL_2_";
  ...
  resultReportData["deforestationAlertsContext"] = await deterTools.getContextDeforestationAlerts(
    resultReportData.property["deforestationAlerts"]
  );
};
...
formatValuesDeter = (resultReportData) => {
  ...
  resultReportData.property.areaPastDeforestation = formatter.formatHectare(
    resultReportData.property.areaPastDeforestation
      ? resultReportData.property.areaPastDeforestation
      : 0
  );
  ...
  for (const point of points) {
    const gsConfig = {
      ...
      cql_filter: `RED_BAND>0;
        rid='${carRegister}';
        gid_car='${carRegister}';
        ${groupViews[type.toUpperCase()].children[groupType[type]].tableName}_id=${
        point.a_carprodes_1_id
      }`,
      format: "image/png",
      height: config.geoserver.imgHeight,
