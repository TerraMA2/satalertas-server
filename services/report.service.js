...
setReportFormat = async function(reportData, views, type, carColumn, carColumnSema) {
  ...
  if (type === 'prodes') {
    ...
    resultReportData['urlGsImage4'] = `${confGeoServer.baseHost}/wms?service=WMS&version=1.1.0&request=GetMap&layers=${views.STATIC.children.CAR_VALIDADO.workspace}:${views.STATIC.children.CAR_VALIDADO.view},${views.PRODES.children.CAR_X_PRODES.workspace}:${views.PRODES.children.CAR_X_PRODES.view}&styles=&bbox=${resultReportData.property.bbox}&width=400&height=400&time=P1Y/${currentYear}&cql_filter=${carColumn}='${resultReportData.property.register}';${carColumnSema}='${resultReportData.property.register}'&srs=EPSG:4674&format=image/png`;
...
module.exports = FileReport = {
  ...
  async getSynthesisCarData(query) {
    ...
    try {
      ...
      const sqlConservationUnitFOCOSCount = `SELECT COUNT(1) AS count FROM public.${views.BURNED.children.CAR_FOCOS_X_UC.table_name} where ${views.BURNED.tableOwner}_${columnCar} = '${carRegister}' ${dateSql}`;
      ...
      const sqlConsolidatedUseFOCOSCount = `SELECT COUNT(1) AS count FROM public.${views.BURNED.children.CAR_FOCOS_X_USOCON.table_name} where ${views.BURNED.tableOwner}_${columnCar} = '${carRegister}' ${dateSql}`;
