...
getSqlAnalysisTotals = async (params) => {
  ...
  for (let analysis of analysisList) {
    ...
    if (analysis.viewId && analysis.viewId > 0 && analysis.viewId !== "null") {
      const table = {
...
getAnalysisChartSql = async (analysis, params) => {
  ...
  if (analysis.viewId && analysis.viewId > 0 && analysis.viewId !== "null") {
    ...
    const columnsFor2 =
      analysis.groupCode && analysis.groupCode === "BURNED_AREA"
        ? `   (CASE
    ...
    const sqlGroupBy2 = ` GROUP BY ${
      analysis.groupCode && analysis.groupCode === "BURNED_AREA" ? subtitle : columns.column2
    }  `;
    const sqlOrderBy = ` ORDER BY ${value} DESC `;
    const sqlLimit = ` LIMIT ${limit} `;

    sql1 += ` SELECT ${columnsFor1} ${sqlFrom} ${filter.secondaryTables}
          ${filter.sqlWhere}
          ${sqlGroupBy1}
          ${filter.sqlHaving}
          ${sqlOrderBy}
          ${sqlLimit}`;
    sql2 += ` SELECT ${columnsFor2} ${sqlFrom} ${filter.secondaryTables}
          ${filter.sqlWhere}
          ${sqlGroupBy2}
          ${filter.sqlHaving}
          ${sqlOrderBy}
...
module.exports.getAnalysis = async (params) => {
  // TODO: Trocar o viewService.getSidebarLayers() abaixo por um método que retorne uma
  //          lista de groupViews (de preferencia apenas os grupos que contém viewGraph = true) e seus filhos
  const sidebarLayers = await viewService.getSidebarLayers();
  const groups = await groupService.get({ dashboard: true });
  if (!groups) {
    throw new InternalServerError("No layers were found");
  }
  for (let idx = 0; idx < groups.length; idx++) {
    const group = groups[idx];
    const params = { groupId: group.id, listSublayers: true };
    const children = await groupViewService.getByGroupId(params);
    let primaryLayer = children.find(
      (layer) => layer.isPrimary && layer.type === LayerTypeName.ANALYSIS
    );
    let analysisList = children.map((layer) => {
      return {
        viewId: layer.viewId,
        code: layer.code,
        ...
        isAnalysis: layer.type === LayerTypeName.ANALYSIS,
        tableOwner: layer.tableOwner,
        tableName: layer.tableName,
      };
    });
    if (primaryLayer) {
      group.viewId = primaryLayer.viewId;
      group.tableName = primaryLayer.tableName;
      group.groupCode = primaryLayer.groupCode;
    }
    group.alert = 0;
    group.area = 0;
    group.selected = group.code === "CAR_X_DETER";
    group.activeArea = group.code === "CAR_X_DETER";
    group.activeAlert = false;
    group.analysisCharts = analysisList;
    group.isAnalysis = true;
  ...
  let analysisList = sidebarLayers
    ...
    .map((layerGroup) => {
      ...
      const primaryLayer = children.find(
        (layer) => layer.isPrimary && layer.type === LayerTypeName.ANALYSIS
      );
      ...
      return {
        viewId: primaryLayer.value,
        code: primaryLayer.cod,
...
module.exports.getAnalysisCharts = async (params) => {
  ...
  for (const analysis of analysisList) {
    const labelChart1 =
      analysis.groupCode === "BURNED"
        ? "Quantidade de alertas de focos por CAR"
        : "Área (ha) de alertas por CAR";
    const labelChart2 =
      analysis.groupCode === "BURNED"
        ? "Quantidade de alertas de focos por Bioma"
        : "Área (ha) de alertas por classe";
